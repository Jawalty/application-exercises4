...
The x axis are proportionate to scale of that specific datam, for better visualization
**Task 1.2:** How is the story this visualization tells different from the original plot?

**Your answer:**

**Original plot emphasizes:** The size of the graphs since the x-axis are the same.

**Free scales plot emphasizes:** It emphasizes the values on the x-axis sinc the graph sizes are the same.
...
The original plot would be misleading because you'd have to make guesses solely based on the size of the graphs.
...
**Original plot (counts) shows:** Number values

**Proportion plot shows:** Percentage values

**Which regions have the most "Wrong" responses as a proportion?** Scotland
...
**Your answer:** It swaps the axis of the values, and organizes it neatly.

**Faceted plot makes it easier to:** I don't see any real benefit that the faceted plot has in comparison to the dodged. I assume it's for individuals who prefer top view scales/values left and right, rather than top bottom.

**Dodged plot makes it easier to:** View all of the data in a more organized manner. You can compare the graphs muich easier since they are up and down, as well as in line with each otherl.
**Task 3.3:** Which region shows the strongest opinion one way or the other (least "Don't know")?

**Your answer:** Scotland also has the least "don't know" values.
...
I would use the dodged plot because I prefer to view my data top down, from a left to right spread. It makes comparison easier since the size of the graphs are in line with each other.
...
The scales free_x visualization would be the most ideal for absolute numbers, because the data are given numerically, rather than percentages.
**Task 4.3:** What's one thing you learned about how visualization choices affect the story you tell?

**Your answer:**

The axis are quite important to keep your data organized and visually neat. Comprehension matters when comparing data, and the more intricately you express it, the more depth you can achieve.
...
brexit_prop %>%
  ...
  ggplot(aes(x = region, y = prop, fill = opinion)) +
  geom_col() +
  labs(
    title = "Proportion who said 'Wrong' by Region",
    subtitle = "YouGov Survey Results, 2-3 September 2019",
    caption = "Source: bit.ly/2lCJZVg",
    x = "Region",
    y = "Proportion",
